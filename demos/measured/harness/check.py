...
Types:
  ...
  ink      an area, "min": at least that many pixels of the area are not the
           ...
           For things that move (orbiting sprites) where a box would pin one
           phase: choose "min" from the smallest count over every phase.
...
def check_ink(c, shot, _ctx):
    ...
    lit = [shot.index(x, y) for y in range(y0, y1 + 1) for x in range(x0, x1 + 1)]
    lit = [i for i in lit if i != bg]
    n = len(lit)
    seen = sorted(set(i for i in lit if i is not None), key=lambda i: -lit.count(i))
    msg = f"{n} pixels of ({x0}, {y0}) to ({x1}, {y1}) are not {colour_name(bg)}"
    msg += " (" + ", ".join(f"{lit.count(i)} {NAMES[i]}" for i in seen[:4]) + ")" if seen else ""
    problems = []
    if n < c["min"]:
