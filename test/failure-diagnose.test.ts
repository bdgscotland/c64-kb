...
describe("failureDiagnose", () => {
  ...
  beforeAll(async () => {
    ...
    await f.addCrashPattern({
      ...
      diagnosis_steps:
        "Count active sprites per line; verify IRQ fires before sprite Y DMA window; check badline overlap.",
    });
    ...
    await f.addCrashPattern({
      ...
      diagnosis_steps:
        "Verify sprite priorities; check IRQ entry jitter with stable-raster double-IRQ trick.",
    });
