...
Two register-level faults in a first C64 soft-scroller. The $D016 XSCROLL
field is global: the VIC-II has no per-row scroll register. The same
register carries the CSEL and MCM bits, which set column width and graphics
mode. Both pitfalls below come from treating $D016 as a dedicated scroll
register.
...
when XSCROLL wraps from 0 back to 7, the static text rows snap back by 7
pixels: XSCROLL going 0->7 moves every row 7 px, and only the
...
In the main loop, update only the data byte of the scroll_set slot. Do not
write $D016 anywhere outside the raster IRQ. The write is op 1:
...
**Region:** both
**Triggered by registers:** D016
**Triggered by techniques:** soft_scroll_h, infinite_scroll_h, char_scroll_buffer_h, parallax_dual_layer, raster_split_modes, multicolor_bitmap, mcm_text, big_font_2x2, dycp_scroller, eight_way_scroll_double_buffer, mode7_lookalike, mci_interlace_bitmap, pseudo_3d_road_raster, tech_tech_wobbler
...
works; $C8 is what the register reads back after the KERNAL's boot
