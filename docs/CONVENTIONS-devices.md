...
| `**VICE attach:**` | How x64sc attaches it: `default` (attached with no option; quote the `-dumpconfig` line in the prose), `flags <x64sc options>` (the exact options, e.g. `flags -controlport2device 2`), `disk` (a runs.json `"disk"`, a D64 in drive 8), `disk d81` (a runs.json `"disk"` with `"type": "d81"`, a D81 in a 1581 as drive 8), `disk 9` (a runs.json `"disk9"`, a D64 in a 1541-II as drive 9) or `crt <n>` (a runs.json `"cartridge"` whose `.crt` header gives hardware type n). |
...
the run or be a `default` device. A run with a D64 `"disk"` whose flags
set `-drive8type` to anything but 1542 fails, since `disk_1541_ii` would
no longer describe it; with a D81 the verifier sets `-drive8type 1581`
itself and any `-drive8type` in the flags fails. A second drive comes
only from `"disk9"`: `-9` or `-drive9type` in the flags fails. A
cartridge's type is read from the built `.crt`.
