...
not. Oscar64 compiles each add to the `clc / lda / adc / sta / lda /
adc / sta` shape that `techniques/maths.md` measures at 26 cycles with
absolute operands; here the compiler keeps both values in zero-page
temporaries and folds the gravity into an immediate (`adc #$20`, then
`adc #$00` for the high byte, read from the `.asm` file the build
writes), which is 20 cycles by the instruction table (rung 3, not
measured in this recipe). The pixel byte is `y_fp >> 8`, one byte move,
