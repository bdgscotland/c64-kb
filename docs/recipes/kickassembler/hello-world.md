...
jumptable. The zero terminator matches C-string layout. BASIC strings on
the C64 are NOT zero-terminated, so this buffer is not BASIC-string
compatible; the convention is this recipe's own.

`.encoding "petscii_upper"` is there on principle, not because this
string needs it. KickAssembler's default text encoding is
...
the two coincide: `screencode_mixed` maps `A`-`Z` to $41-$5A,
...
KickAssembler 5.25 + VICE 3.10). The difference shows as soon as the letters
