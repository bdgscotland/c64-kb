...
conventional assembler with a JavaScript-like script language that runs at
assembly time, so sine
tables, sprite coordinate arrays and other data are generated in the source file
without an external tool. Script variables, constants and loops interleave
with mnemonics; there is no separate pre-pass.

**Targets:** 6510

KickAssembler is the second tool in the c64-kb toolchain ranking. Use it
when a task needs cycle-counted raster effects, sprite
multiplexing, FLI/AFLI modes, or a full-assembly cracktro where every cycle
counts. Where
readability, ease of refactoring and C-level abstractions matter more than
a few cycles per scanline, use Oscar64.

## Quick reference

Install a Java runtime. Java 11 or later is recommended, but the vendor's
...
`http://theweb.dk/KickAssembler/`, unzip it, and
assemble with:
...
The default output. KickAssembler prepends a two-byte load address (little-
...
for code that is not loaded by the KERNAL (cartridge ROMs, raw block
copy targets).
...
KickAssembler creates D64 images with the `.disk` directive, which
wraps the CC1541 library. One image can mix assembled segments and files from the
host filesystem.
...
or `!label+` (next). Bare `!:` and `!+` / `!-` work for short forward/backward
jumps:
...
Use it for almost everything: shared macros, constants,
...
bytes. Use it when the module is large and rarely changes (a
...
The `.sym` holds addresses only: no macros, no `.const` values, no
code. After the module is rebuilt, the caller's copy of the
addresses is stale until the caller is assembled again, so a build script
assembles the module first and the caller second. For the `.sym` and
...
- The task is text-mode utility work that the cc65 standard library
  covers.
...
- You are saving 5–10 cycles per scanline on code that runs once per frame
  at non-display time; the saving does not pay for the harder maintenance.
...
vice-mcp uses the same `.vs` file format: it sets breakpoints by label, reads
memory by symbolic address, and annotates register dumps with
labels, so the agent loop needs no hard-coded addresses.
...
convenience, not as a stated minimum; it does not mean Java 8 is
refused. It runs here under Temurin 26; Java 11/17/21 LTS are the usual
...
token` / `Pseudo command '...' not defined` unless you pass `-pseudoc3x`. The failure
is an error with no output; an earlier version of this page said it "fails
silently or misassembles". When adapting 3.x code from CSDB or similar
archives, check the separator.

**`LoadPicture` accepts GIF and JPEG as well as PNG.** The manual's
examples use GIF files. PNG works too; the function is not PNG-only.
...
KickAssembler does not. The comment that does not
fail is worse: `; inc $d020` assembles the `inc` (measured on 5.25, exit 0), so a
