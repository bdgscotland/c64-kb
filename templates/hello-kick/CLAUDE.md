...
and uses the meter's macros (`#import "frame_meter.asm"`). Start a pure
KickAssembler program from it with `npm run new-project -- hello-kick <dir>`
in c64-kb. `PLAN.md` here is a plan filled from real tool output;
new-project keeps it as `PLAN-hello-kick-example.md` and gives the new
program a blank one.
