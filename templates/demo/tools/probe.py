...
usage: probe.py probe-pal.png probe-ntsc.png [more pairs ...]
...
def main():
    ...
    for model, paths in shots.items():
        ...
        print(f"{model}: {len(paths)} shot(s), $D021 store column {sorted(body, key=str)}, last chunk {sorted(last, key=str)}")
        ...
        xl = (next(iter(last)) - SHIFT_PX) if len(last) == 1 else x2 + 16
        after = x1 + line_px - 383           # pixels past the line above's last visible pixel
        print(f"{model}: without the probe, $D020 at x {x1} and $D021 at x {x2} ({xl} in the last chunk), "
              f"x 0 being the line's first visible pixel; the $D020 store is {after} pixels after the "
              f"line above leaves the picture")
