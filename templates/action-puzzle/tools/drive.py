...
"""drive.py PRG STEP...: play the `make joy` build headless over VICE's binary monitor.
    ...
    python3 tools/drive.py build/action-puzzle-joy.prg "until:FIRE TO START" tap:fire \
        until:LIVES hold:right "until:ENTER YOUR NAME" tap:up tap:fire tap:fire tap:fire \
        "until:GAME OVER" print

The joy build reads its port byte from $02FE (JOY_SOURCE) instead of $DC00,
because the windowless VICE's joyport commands never reach $DC00 (see
README). Steps:
...
screenshots came back stale while a game was running.
"""
import os
import socket
import struct
...
class Vice:
    ...
    def screen(self):
        d = self.cmd(0x01, struct.pack("<BHHBH", 0, 0x0400, 0x07e7, 0, 0))[2:]
