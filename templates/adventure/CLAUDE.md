...
## This starter: adventure

STARWATCH, a two-word text adventure in Oscar64 C: twelve rooms, containers,
three puzzles, a scored ending, SAVE and LOAD to drive 8, packed text and a
multicolour picture strip. README.md has the file map and the measured
frame. Start a program from it with `npm run new-project -- adventure <dir>`
in c64-kb. `PLAN.md` here is a plan filled from real tool output;
new-project keeps it as `PLAN-adventure-example.md` and gives the new
program a blank one.

Rules this starter adds:

- The world lives in `tools/world.py`. Edit it, run `make gen`, then
  `make shot check`. Never edit the generated `src/gen_*.h`.
- The engine and the printer live twice: `src/engine.c` and `src/text.c`,
  and the model in `tools/gen.py`. Change both. The verdict compares the
...
- Disk calls run on a frame outside the meter, muted, followed by `SEI`.
  Run `make disktest` after any change to `save.c` or the save record.
