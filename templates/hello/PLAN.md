...
The minimal starter. It exists to prove the harness end to end, so its plan
...
sprite registers are written by a KickAssembler routine that C calls, and the
frame's work is timed by the harness meter. PAL and NTSC.
...
|---|---|---|---|
| frame_sync_loop | one pass of the loop a frame, synced on raster line 250 | oscar64-frame-sync-loop | badline_cycle_loss, d012_wrap_around, raster_line_count_difference, vic_colour_register_upper_nibble_reads_set, and the rest of its list |
...
play frames): worst 194 cycles, typical (the median) 172, the same on PAL and
...
- `$0801-$0880` Oscar64 startup; `$0900-$0FFF` the KickAssembler blob
  (`src/sprite.asm`, 31 bytes now); `$1000` up, Oscar64's code, data and stack.
...
- The KickAssembler routine takes its arguments in its own bytes
  (`ASM_SPR_X`, `ASM_SPR_Y`), not Oscar64's zero page, which the compiler owns.
