...
c64-kb harness (`templates/_harness` in c64-kb, or `./harness` when vendored).
...
The smallest program that uses every harness target. `src/main.c` (Oscar64)
runs the loop; `src/sprite.asm` (KickAssembler) places sprite 0; the meter
times each frame. Copy it to start a new program, or build a starter for an
archetype on the same harness. `PLAN.md` shows a plan filled from real tool
output. To use it outside c64-kb, copy `templates/_harness` into `./harness`
and set `C64KB` to your c64-kb checkout.

## Before any code

1. Brief: `npx tsx src/cli.ts game-briefing "<concept>" --archetype <name>`
   (or `demo-briefing`). Read what it proposes.
...
3. Fill `PLAN.md`: paste the whole output of `check-compatibility` and of
   `plan-budget` into their sections, and replace every `FILL:` line.

A hook blocks writes under `src/` and `make` refuses to build until `PLAN.md`
passes that check (`make plan-gate` says what is missing).
`make PLAN_GATE=off` overrides it; say why in `PLAN.md` if you do.
...
- Frame cost comes from the meter: CIA2 timer A, printed as
  ...
  builds. Typical is the mean of the last 16 recorded frames.
...
- Commit named paths only. Never `git add -A` or `git add .`. `build/` and
...
- `AUTOPILOT=1` builds read a scripted joystick byte instead of `$DC00` and
  ...
  on fail. `FORCE_FAULT=1` must turn the verdict red.
...
- A KickAssembler part called from C is a raw blob at its own `* =` address;
  `build/asm.h` gives C its labels as `ASM_<LABEL>`. Pass arguments through
  bytes in the blob, not Oscar64's zero page.
- The meter takes CIA2 timer A. Do not open RS-232 (device 2) while it runs.

## Calling c64-kb

- CLI, from the c64-kb checkout: `npx tsx src/cli.ts <command>`;
  `npx tsx src/cli.ts --help` lists the commands.
- MCP: `.mcp.json` starts `node <c64-kb>/dist/cli.js serve` (run `npm run build`
  in c64-kb first). Fix the path in `.mcp.json` if this project moved.
- `C64KB` points `make claims` at the checkout (default `../..`).
