...
Output: {name, toolchain, output_format, region, source_doc, toolchain_version_verified?, documentation[], source_code?, verified_on[{variant, vic, sid, cia, region, model, cycles, shot, flags, pinned, overrides[]}], claims[{unit, mode, ranges?, relocatable?}], claims_stated, claims_basis?, devices[{name, title, kind, port, vice_attach, claims[{unit, mode}], claims_stated, claims_basis?}], devices_stated}. toolchain_version_verified is the toolchain version this repo's gates built the recipe with (e.g. '5.25' for KickAssembler); a different version may or may not build it. verified_on lists the VICE machine variants verify:recipes runs the recipe on and compares with a committed screenshot pixel for pixel (from docs/recipes/runs.json): a runs.json "pal" run is VICE's default machine, which is the c64c variant (VIC-II 8565, SID 8580, CIA 8521), not a 6569; "ntsc" is the 6567R8 with a 6581 and 6526. vic, sid and cia are the chips the run had: a chip flag in 'flags' (-ciamodel, -cia1model, -cia2model, -sidmodel, -VICIImodel) replaces the variant's chip, and 'overrides' names each one replaced (cia-revision-detect runs the c64c with -ciamodel 0, so its CIA is the 6526). pinned false means the run uses verify:recipes' defaults. An empty list means no run is compared. claims are the hardware units the listing itself chooses beyond its techniques' claims, from the page's claims: frontmatter (the IRQ vector it installs, its zero-page bytes; basis measured-vice: written from a claims-watch trace); claims_stated is 'unknown' when the page has no claims: key, which is not 'claims nothing'. devices are what the recipe's pinned run attaches (a 1541-II as drive 8, a joystick, paddles, an EasyFlash or Magic Desk cartridge, a REU), from its devices: frontmatter and docs/hardware/devices.md: port is control_1, control_2, user, expansion or serial; vice_attach is how x64sc attaches it (default, flags <options>, disk, or crt <CRT hardware type>); claims are the units the device owns or shares. devices_stated is 'none' for a recipe that needs nothing beyond the stock machine and 'unknown' when the page has no devices: key. On not-found, 'name' is empty and 'text' lists near-match suggestions.
...
Returns structured: {name, title, category, complexity, chip?, requires_region?, uses_registers[], uses_kernal[], recipes[], requires[{name,title}], required_by[{name,title}], alternatives[{name,title,tradeoff,stated_on}], mitigates[{name,title,severity}], documentation[], cost?}. requires/required_by are direct REQUIRES edges authored from **Requires:** lines (CONVENTIONS-techniques.md); a variant of a technique (double_irq of stable_raster_irq) is not a prerequisite and does not appear here. alternatives are ALTERNATIVE_TO edges in either direction (schema 37), authored from **Alternative to:** lines: techniques that do the same job another way, e.g. sprite_multiplex_8 for sprite_multiplex_24; 'tradeoff' describes 'stated_on' (the technique whose page states the pair) against the other, in the page's words. 'cost' is present only when the page carries a **Cost:** line: {cycles_per_line?, cycles_per_frame?, lines_active?, bytes_code?, bytes_data?, zp_bytes?, irq_slots?, basis, bytes_basis?}, integers, with only the keys the page stated; 'basis' is one of measured-vice, derived-listing, arithmetic, estimated and says how the figures were obtained (measured-vice means run in VICE; estimated means a judgement). 'bytes_basis', present only when the page states a **Cost bytes basis:** line, is the basis of bytes_code, bytes_data and zp_bytes, and 'basis' then covers the other figures. Cycles are per PAL frame of 19,656 unless the page says otherwise; bytes are the built recipe's segments. 'claims' [{unit, mode, ranges?, relocatable?}] lists the HardwareUnits the technique holds (**Claims:** line; units such as sid_voice_2, sprite_0..7, vic_raster_irq, irq_vector_0314, zero_page with ranges like "E0-EF"); mode is owns, shares (writes under the owner's protocol), reads or init (once, before the frame loop). 'claims_stated' is stated, none (claims no unit) or unknown (the page states nothing: never read unknown as none); 'claims_basis' is measured-vice, derived-listing or estimated.`,
  inputSchema: {
    name: z
      .string()
...
Conflict kinds: cpu_exclusive, cpu_vs_irq, sprite_set, kernal_banked_out, serial_bus_busy (a resident fast loader against KERNAL disk I/O), region_mismatch (hard); sprite_set is soft when the constant-set side meets one that claims sprites and does not change them mid-frame: those sprites can be its constant set, counted in its timing (#90; sideborder_open beside sprite_border_scroller); shared_register, shared_kernal (soft); prerequisite_conflict (the severity of its underlying_kind).

State left running against KERNAL disk I/O (#94), between any two inputs and, in a phased check, between members of different phases (such a finding has no 'phase' and carries across{a_phase, b_phase}; state goes forward only, so nothing play or a transition leaves running is checked against an init member):raster_irq_during_serial_io (soft: one keeps a raster IRQ, by midframe_raster_irqs or a vic_raster_irq claim; the other calls a routine the pitfall of that name is triggered by, e.g. OPEN, CHRIN, LOAD); sprites_over_badlines_hang_serial_io (soft: one shows sprites, by changes_sprite_set or a sprite claim; the other calls IECIN, CHRIN, CHKIN, OPEN, CLOSE or LOAD); kernal_banked_out across phases (soft: one phase runs KERNAL-out, another calls it; within one phase it stays hard); recipe_kernal_out (info: a recipe that builds one also implements a kernal_rom_out technique such as ram_under_kernal, or owns irq_vector_fffe, and the other calls the KERNAL; kickassembler-sprite-multiplex-game does this). To state that a design runs with the KERNAL banked out, list ram_under_kernal: it demands kernal_rom_out, so any KERNAL-calling technique beside it is kernal_banked_out.

Unit claims (**Claims:** lines, CLAIMS edges to HardwareUnit nodes): unit_contention (hard: both own the same unit, e.g. two raster-IRQ owners, since there is one raster compare, or two owners of sprite_0..7; soft for the raster compare when irq_chain_table is in the set, since its table then programs $D012 and each owner becomes an entry, so adding the host the resolution names never makes the verdict worse, and soft when both owners' bands, stated or placed, share no line, since one compare then serves both as a chain in which each handler arms the next one's line; an earlier version called that hard, against the one-part-demo and fli-music-scroller recipes); zero_page_overlap (hard, soft when either side is relocatable; 'shared' lists the bytes); unit_shared (soft: one owns and the other shares, e.g. SFX on voice 2 beside a music player, or stable_raster_irq, a way into a handler, beside a raster effect that owns the compare; a resident loader beside a VIC bank owner says not to write $DD00 raw); unit_read_while_driven (soft: one reads a port the other drives); init_order (info: one uses the unit once at start-up; info does not change the verdict). Between a technique and its own prerequisite, unit_contention and zero_page_overlap do not run, and unit_shared runs only when the sharer is the one that requires the owner. A prerequisite's claims on units its input holds are the input's and are not reported again. kernal_clobbers_zp (soft): one technique calls a KERNAL routine (**Uses kernal:**) whose zero-page may set (CLOBBERS_ZP, a static walk of the ROM through the power-on vectors, an upper bound) overlaps bytes the other claims; 'shared' lists the bytes and the routines. It also runs inside one input's own chain, the technique against itself and against its own prerequisites, since a KERNAL call clobbers the bytes whoever declared them; such a finding has a = b = that input (a prerequisite_conflict with the prerequisites in 'via' when they are involved). A routine hit already reported for the same inputs, routine and bytes is not repeated through a prerequisite. recipe_zero_page_overlap (info, schema 34): a recipe that implements one input and a recipe that implements the other (neither implementing both) own zero-page bytes in common, from their claims: frontmatter; 'shared' lists the bytes and the rationale names the recipe pairs. The techniques do not conflict; the listings would if both were copied into one program. The interrupt vector a recipe installs is not compared. recipe_device_conflict (info, schema 36): a recipe that implements one input and a recipe that implements the other require devices that cannot both be attached (both own one unit, e.g. expansion_io2 for a REU and an EasyFlash, or both sit in one single-socket port: two cartridges, a joystick and paddles in control port 2); 'shared' lists the units and ports. data_coverage carries claims: stated | none | unknown per technique, and the text says for how many inputs claims are stated; an unknown claim set is never treated as "claims nothing", so a unit conflict with it cannot be ruled out.
...
Raster bands: a technique page may state the raster lines it holds the CPU on (**Raster band:**, e.g. fli_image 45-250). When both techniques state line bands and they share no line, the line-sharing rules (cpu_exclusive, cpu_vs_irq through mid-frame IRQs or sprite-set changes, sprite_set) do not fire; the pair is listed in band_separated[] {a, b, a_band, b_band, rules[]} instead. A missing band, or one the program chooses ('movable'), keeps the conflict, and its rationale names the unknown side. "name@lines" places such a technique for this check (a recipe's raster_bands: frontmatter says where its trace ran it): the rules then read the placed lines as stated, data_coverage carries them as placed_band, and {"techniques": ["sideborder_open@248-272", "raster_bars@17-50"]} → warnings where the bare names are incompatible. A placement on a technique whose page states lines, or outside the grammar, is not used: it is listed in placements_refused[] {input, why} and the page's band stands. data_coverage carries each technique's raster_band.

Limitations: demands and prerequisites are authored per technique in docs/techniques (see CONVENTIONS-techniques.md); a technique without them only participates in the soft checks. Named techniques are checked as named even when one requires the other, except for the two line rules above. Where bands are not stated, 'incompatible' means 'not on the same raster lines', and the resolution says so.`,
  inputSchema: {
    techniques: z
      .array(z.string())
      .optional()
      .describe(
        "Array of 2+ canonical technique names to check (e.g. ['stable_raster_irq', 'raster_bars']), each 'name' or 'name:phase' (any phase makes the check phased), optionally with '@lines' placing a movable band ('sideborder_open@248-272'); with 'design', optional",
      ),
    design: z
      .string()
      .min(1)
...
Inputs: 'techniques' is a list of canonical technique names, each optionally with a phase: "name" (play), "name:play", "name:transition" (level decode, wipe) or "name:init" (one-off setup). Each phase is budgeted alone. A name may carry calls per frame, "name ×N" or "name ×M-N" ("*" for "×" works), before the phase: a Cost figure is one call, so a cycles_per_frame charge is multiplied, low by M and high by N; the multi-frame test is on one call. On a technique whose Cost states cycles_per_item (char_bullets, per_frame_hitbox; #95) the count is items, not calls, and M may be 0: the charge is cycles_item_base + M × item low and + N × item high, charge "per_item", never multi-frame; without a count it is the recipe's own cycles_per_frame. 'design' is a GameDesign name (a whole game from docs/game-design/designs, e.g. "platformer_scaffold_oscar64"): its COMPOSES edges, each in its phase, are budgeted first, then any 'techniques' given; its region is the default. Give 'techniques', 'design' or both. 'region' is 'pal', 'ntsc' or 'both' (default: PAL unless every region-locked member is NTSC-locked). 'screen' is 'on' (default) or 'off'. 'sprites_per_line' (0-8) and 'sprite_lines' (default 200) charge sprite DMA, 3 + 2n cycles a line.

Output: {design{name, title, region, instance_of[], realised_by[], composes[{technique, phase, calls?{low, high}}], source_doc, measured[{phase, region, worst, typical, basis, source, predicted{low, high, fixed, verdict, missing[]}, position: below_low | within | within_incomplete | above_high | not_predicted, finding}]} | null, design_not_found?{requested, known[]}, techniques, refused[], region, screen, sprites, phases[{phase, region, frame, contributors[{name, low, high, every_frame, basis, charge: cycles_per_frame | per_line | band | per_item, calls?{low, high}, per_item?{base, each}, measured_on, conditions}], excluded[{name, reason: multi_frame | included_by | inside_band_of, by?, cycles?}], unknown[], not_found[], to_measure[{technique, recipe, why}], fixed_losses{badlines, sprite_dma, charged_for[], badlines_in_bands, floor}, worst_only[], low, high, floor, verdict, weakest_basis, irq_slots, notes[]}], bytes{sum, contributors[{name, bytes, basis}], excluded (whole_program), inside[{name, by}], without_bytes, weakest_basis}, verdict, assumptions[]}. A phase's weakest_basis is over its cycle figures only; bytes.weakest_basis is over the byte figures, each taken from the page's **Cost bytes basis:** line where it has one, else its **Cost basis:**. A design's position places its measured worst against [low, high + fixed]: within_incomplete is inside that range while predicted.missing is non-empty, so the uncounted members could move the range past it and it is not agreement; above_high with members missing may be explained by them; below_low stands, since a missing member can only raise the range.

Rules: low sums each member's cycles_per_frame_typical where the page states a measured one, else its cycles_per_frame; high sums cycles_per_frame (worst frames). A member with no cycles figure goes to unknown and to_measure, with the recipe to measure it on. A figure above the longest frame (PAL, 19,656) is excluded as multi_frame on both models; one between the two frames is summed on NTSC and counts against its 17,095 (an earlier version judged each model against its own frame, so 18,559 was summed on PAL and dropped on NTSC). A member named in another's **Cost includes:**, followed through the graph (a includes b, b includes c), is excluded as included_by, unless the including member is itself multi-frame; of two members that include each other the first listed is kept. A name listed twice in one phase is counted once and the repeat is listed in refused. A technique that holds every cycle of a stated raster band (cycles_per_line 63 and a line band) is charged band lines × line length, and its REQUIRES closure in the set is excluded as inside_band_of. With the screen on, the badlines (lines 51-243, every eighth, 25 × 43 = 1,075 cycles) outside any band charge, and the stated sprite DMA, are charged as fixed losses unless every summed figure is a band charge or is measured with measured-on conditions that say the screen was on; a figure measured blanked, one that does not say, and an arithmetic, derived-listing or estimated one are all charged; a figure of zero cycles is not. A stall takes its cycles wherever the code runs, so the charge is exact when no summed figure already holds stalls and too high by what a screen-on figure holds; fixed_losses.floor is the part no figure can hold. The low end is not a floor: a typical figure is a common frame or a real run's worst, and worst_only lists members with no typical at all (a zero worst needs none). floor is the band and per-line charges, which run every frame, plus fixed_losses.floor. Verdict per phase: fits when nothing is unknown, missing or multi-frame and high + fixed losses fit the frame; over when floor is more than the frame; otherwise undetermined. The overall verdict is the worst phase's. Bytes flagged "(whole PRG)" on their measured-on line are not summed; a member whose work is inside another's figure that states bytes is listed in inside, not summed. 'region' other than pal, ntsc or both is refused.

Examples: {"techniques": ["wave_director", "object_pool"]} → object_pool excluded (included_by wave_director); 1,170-3,188 plus 1,075 fixed, floor 1,075; fits. {"techniques": ["fli_image", "stable_raster_irq", "double_irq"]} → fli_image charged 207 lines × 63 = 13,041; the two prerequisites inside its band; all 25 badlines are inside the band, so no fixed loss. {"techniques": ["soft_scroll_h", "sid_play_routine_pattern"]} → 7,938 + 1,223 = 9,161 plus 1,075 fixed; fits (soft_scroll_h's carry frame; before issue #18 it was a 74,041-cycle move, excluded as multi_frame). {"design": "falling_blocks_oscar64", "region": "pal"} → init and play phases; play undetermined (text_mode_overlay_render's worst, a lock frame's full repaint, takes the high end past the frame); the design's measured PAL worst frame, 6,276 cycles, is below the predicted range, whose low end holds falling_block_rules' constructed upper bound.
