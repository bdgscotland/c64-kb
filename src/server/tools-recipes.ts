...
Examples: {"techniques": ["wave_director", "object_pool"]} → object_pool excluded (included_by wave_director); 1,170-3,188 plus 1,075 fixed, floor 1,075; fits. {"techniques": ["fli_image", "stable_raster_irq", "double_irq"]} → fli_image charged 207 lines × 63 = 13,041; the two prerequisites inside its band; all 25 badlines are inside the band, so no fixed loss. {"techniques": ["soft_scroll_h", "sid_play_routine_pattern"]} → 7,938 + 1,250 = 9,188 plus 1,075 fixed; fits (soft_scroll_h's carry frame; before issue #18 it was a 74,041-cycle move, excluded as multi_frame). {"design": "falling_blocks_oscar64", "region": "pal"} → init and play phases; play undetermined (text_mode_overlay_render's worst, a lock frame's full repaint, takes the high end past the frame); the design's measured PAL worst frame, 6,276 cycles, is below the predicted range, whose low end holds falling_block_rules' constructed upper bound.
