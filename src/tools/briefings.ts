 ...
 */
async function resolveProposedTechniques(description: string, limit = 8): Promise<string[]> {
...
export function whyProposed(techniqueName: string, category: string, description: string): string {
  ...
  if (
    (name.includes("scroll") || name.includes("soft_scroll")) &&
    (desc.includes("scroll") || desc.includes("scroller"))
  ) {
    return "Scroller effect mentioned in brief";
  }
  if (
    (name.includes("sprite") || name.includes("multiplex")) &&
  ...
  if (
    (name.includes("raster") ||
      name.includes("bar") ||
      name.includes("double_irq") ||
      name.includes("stable_raster")) &&
...
async function resolveArchetype(raw: string, preferKind: "game" | "demo"): Promise<ArchetypeResolution> {
  ...
  return {
    ...
    features: ((f.data ?? []) as { name: string }[]).map((x) => x.name),
    risks: ((r.data ?? []) as { name: string }[]).map((x) => x.name),
...
export function computeBudget(techs: BudgetInput[], regionHint?: "PAL" | "NTSC"): BriefingOutput["budget"] {
  const locked = techs
    .map((t) => (t.requires_region ?? "").toUpperCase())
    .filter((r) => r === "PAL" || r === "NTSC");
  const region: "PAL" | "NTSC" =
  ...
  for (const t of techs) {
    ...
    const c = t.cost;
    const bytes =
      c.bytes_code !== undefined || c.bytes_data !== undefined
  ...
  const assumptions = [
    ...
    ...(is_floor
      ? [`${without_cost.length} proposed technique(s) have no Cost line, so both sums are floors.`]
      : []),
  ];
  return {
    region,
...
async function buildBriefing(
  ...
  const resolved: ArchetypeResolution | undefined =
  ...
  const compatibility: BriefingOutput["compatibility"] = {
    ...
    warnings: compatResult.structured.conflicts.filter(
      (c) => c.kind === "shared_register" || c.kind === "shared_kernal",
    ),
  ...
  const pitfallsRaw: { name: string; title: string; severity: string; triggered_by_proposed: string[] }[] =
    [];
  ...
  if (resolved?.mode === "graph" && resolved.risks.length > 0) {
    ...
    for (const row of (rows.data ?? []) as {
      ...
      pitfallsRaw.push({
        ...
        triggered_by_proposed: (row.triggers ?? []).filter(
          (t) => t && validTechs.some((vt) => vt.name === t),
        ),
      });
    }
  }
  ...
  const pitfalls = pitfallsRaw.sort(
    (a, b) =>
      (SEVERITY_ORDER[a.severity] ?? 4) - (SEVERITY_ORDER[b.severity] ?? 4) || a.name.localeCompare(b.name),
  );
  ...
  for (const row of (demandRows.data ?? []) as { name: string; demands: string[] }[]) {
    demandsOf.set(row.name, (row.demands ?? []).filter(Boolean));
  }
  const cycloTightTechs = validTechs.filter(
    (t) =>
      (demandsOf.get(t.name) ?? []).some((d) => CYCLE_TIGHT_DEMANDS.has(d)) || t.complexity === "scene-tier",
  );
  ...
  if (isGame) {
    ...
    if (resolved?.mode === "graph") {
      ...
      for (const row of (rr.data ?? []) as { name: string; source_doc?: string }[]) {
      ...
      const rr = await fk.roQuery(
        `MATCH (r:Recipe {name: "oscar64-simple-shmup"}) RETURN r.name AS name, r.source_doc AS source_doc`,
      );
...
function renderBriefingText(
  ...
  if (b.archetype_not_found) {
    out +=
      `**Archetype not found:** "${b.archetype_not_found.requested}".` +
      (b.archetype_not_found.candidates?.length
        ? ` Did you mean one of: ${b.archetype_not_found.candidates.join(", ")}?`
        : "") +
