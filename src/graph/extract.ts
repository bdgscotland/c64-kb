...
export type GraphEntity =
  ...
  | { type: "technique"; name: string; title: string; category: string; complexity?: string; chip?: string; cost?: TechniqueCost; cost_basis?: CostBasis; raster_band?: string; claims_stated?: "stated" | "none"; claims_basis?: ClaimsBasis }
  | { type: "claims"; owner: string; ownerKind: "Technique"; unit: string; mode: ClaimMode; ranges?: string; relocatable?: boolean; basis: ClaimsBasis }
...
export type HardwareUnitKind =
  | "sid_voice" | "sid_shared" | "sprite" | "timer" | "tod" | "port" | "bus"
  | "irq_source" | "vector" | "io_page" | "zero_page";
...
export const HARDWARE_UNITS: readonly HardwareUnit[] = [
  ...
  { name: "cia1_port_a", kind: "port", addresses: "$DC00 (keyboard column drive, control port 2, POT select)", chip: "CIA1" },
  ...
  { name: "vic_raster_irq", kind: "irq_source", addresses: "$D012, $D011 bit 7, $D019/$D01A bit 0", chip: "VIC-II" },
...
export function parseClaims(raw: string): Claim[] | { error: string } {
  ...
  for (const item of items) {
    ...
    let modeSeen = false;
    for (const word of (mods ?? "").split(",").map((w) => w.trim()).filter((w) => w !== "")) {
    ...
    for (const u of units) {
      if (!HARDWARE_UNIT_NAMES.has(u)) return { error: `"${u}" is not a hardware unit (see docs/ONTOLOGY.md, HardwareUnit)` };
    }
    if (units.includes("zero_page")) {
      ...
      seen.add("zero_page");
      claims.push({ unit: "zero_page", mode, ranges: formatZeroPageRanges(zp), ...(relocatable ? { relocatable: true } : {}) });
      continue;
    }
...
export function extractGraphEntities(content: string, sourcePath: string): GraphEntity[] {
  ...
  if (content.includes(TECHNIQUE_MARKER)) {
    ...
    let pendingMeta: { region?: string; usesReg?: string[]; usesKernal?: string[]; demands?: string[]; requires?: string[]; cost?: TechniqueCost; costBasis?: string; rasterBand?: string; claims?: Claim[]; claimsRefused?: boolean; claimsBasis?: string } = {};
    ...
    const flush = () => {
      ...
      if (pendingMeta.claims !== undefined) {
        ...
        if (basis === undefined) {
          console.warn(`[extract] ${sourcePath}: technique ${currentTech.name} has a **Claims:** line but no **Claims basis:** line — Claims not ingested, so its claims read as unknown (see CONVENTIONS-techniques.md)`);
        } else if (!(CLAIMS_BASIS_WORDS as readonly string[]).includes(basis)) {
          console.warn(`[extract] ${sourcePath}: technique ${currentTech.name} has claims basis "${basis}", which is not one of ${CLAIMS_BASIS_WORDS.join(", ")} — Claims not ingested (see CONVENTIONS-techniques.md)`);
        } else {
        ...
        console.warn(`[extract] ${sourcePath}: technique ${currentTech.name} has a **Claims basis:** line but no **Claims:** line — ignored`);
      }
      entities.push({
        type: "technique",
        ...currentTech,
        ...
        ...(claims !== undefined && claimsBasis !== undefined
          ? { claims_stated: claims.length > 0 ? ("stated" as const) : ("none" as const), claims_basis: claimsBasis }
          : {}),
      });
      for (const c of claims ?? []) {
        entities.push({ type: "claims", owner: currentTech.name, ownerKind: "Technique", ...c, basis: claimsBasis! });
      }
      if (currentTech.chip) {
    ...
    for (const line of lines) {
      ...
      const clb = line.match(CLAIMS_BASIS_LINE);
      if (clb) {
        pendingMeta.claimsBasis = clb[1].trim().replace(/`/g, "");
        continue;
      }
      const cl = line.match(CLAIMS_LINE);
      if (cl) {
        const parsed = parseClaims(cl[1]);
        if ("error" in parsed) {
          console.warn(`[extract] ${sourcePath}: technique ${currentTech.name} has **Claims:** ${JSON.stringify(cl[1].trim())}: ${parsed.error} — Claims not ingested, so its claims read as unknown (see CONVENTIONS-techniques.md)`);
