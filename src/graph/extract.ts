...
// ── Resource claims (schema 25) ─────────────────────────────────────────────
// **Claims:** names the pieces of hardware a technique holds while it runs,
// and how (docs/CONVENTIONS-techniques.md); **Claims basis:** says how the
// claims were established. Each unit is a seeded HardwareUnit node. No line
// means "unknown", which is not the same as `none` ("claims no unit").
...
export function parseZeroPageRanges(raw: string): Array<[number, number]> | { error: string } {
  const out: Array<[number, number]> = [];
  for (const part of raw.split("+").map((s) => s.trim())) {
    const m = part.match(/^\$([0-9A-Fa-f]{2})(?:\s*-\s*\$([0-9A-Fa-f]{2}))?$/);
    if (!m) return { error: `"${part}" is not a zero-page byte $XX or range $XX-$YY` };
    const first = parseInt(m[1], 16);
    const last = m[2] !== undefined ? parseInt(m[2], 16) : first;
...
export function formatZeroPageRanges(ranges: Array<[number, number]>): string {
  return ranges.map(([a, b]) => (a === b ? hex2(a) : `${hex2(a)}-${hex2(b)}`)).join(",");
}
/** Inverse of formatZeroPageRanges; returns [] for anything malformed. */
export function zeroPageRangesFromCanonical(canonical: string): Array<[number, number]> {
  const out: Array<[number, number]> = [];
  for (const part of canonical.split(",").filter((s) => s !== "")) {
    const m = part.match(/^([0-9A-F]{2})(?:-([0-9A-F]{2}))?$/i);
    if (!m) return [];
...
export function parseClaims(raw: string): Claim[] | { error: string } {
  ...
  for (const item of items) {
    ...
    const m = item.match(/^([a-z][a-z0-9_]*(?:-\d+)?)(?:\s+(\$[^()]*?))?\s*(?:\(([^()]*)\))?$/);
    if (!m) return { error: `"${item}" is not <unit>[ (<mode>)]` };
    const [, unitWord, addr, mods] = m;
    let mode: ClaimMode = "owns";
    let relocatable = false;
    let modeSeen = false;
    for (const word of (mods ?? "")
      .split(",")
      .map((w) => w.trim())
      .filter((w) => w !== "")) {
      if ((CLAIM_MODES as readonly string[]).includes(word)) {
        if (modeSeen) return { error: `"${item}" names two modes` };
        mode = word as ClaimMode;
        modeSeen = true;
      } else if (word === "relocatable") {
        relocatable = true;
      } else {
        return { error: `"${word}" in "${item}" is not a mode (${CLAIM_MODES.join(", ")}) or "relocatable"` };
      }
    }
    // A unit range: sprite_0-7, sid_voice_1-3.
    let units: string[];
    const range = unitWord.match(/^(.*_)(\d+)-(\d+)$/);
    if (range) {
    ...
    for (const u of units) {
      if (!HARDWARE_UNIT_NAMES.has(u))
        return { error: `"${u}" is not a hardware unit (see docs/ONTOLOGY.md, HardwareUnit)` };
    }
    if (units.includes("zero_page")) {
      if (!addr) return { error: `zero_page needs its bytes, e.g. "zero_page $FB-$FE"` };
      const zp = parseZeroPageRanges(addr);
      if ("error" in zp) return { error: zp.error };
    ...
    if (addr) return { error: `"${item}": only zero_page takes addresses` };
    if (relocatable) return { error: `"${item}": only zero_page can be relocatable` };
    for (const u of units) {
      if (seen.has(u)) return { error: `${u} is claimed twice` };
      seen.add(u);
      claims.push({ unit: u, mode });
    }
  }
  return claims;
}
...
function splitH3Sections(body: string): { heading: string; body: string }[] {
...
function splitH2Sections(body: string): { heading: string; body: string }[] {
  const sections: { heading: string; body: string }[] = [];
  const lines = body.split("\n");
  let currentHeading: string | null = null;
  let currentBody: string[] = [];
...
export function extractGraphEntities(content: string, sourcePath: string): GraphEntity[] {
  ...
  if (content.includes(TECHNIQUE_MARKER)) {
    ...
    if (!TECHNIQUE_CATEGORIES.has(category)) {
      console.warn(
        `[extract] ${sourcePath}: technique doc category "${category}" is not in the ontology's category set — no techniques ingested from this file (see CONVENTIONS-techniques.md)`,
      );
      return entities;
    }
    ...
    const flush = () => {
      ...
      if (pendingMeta.cost !== undefined) {
        ...
        if (basis === undefined) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} has a **Cost:** line but no **Cost basis:** line — Cost not ingested (see CONVENTIONS-techniques.md)`,
          );
        } else if (!(COST_BASIS_WORDS as readonly string[]).includes(basis)) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} has cost basis "${basis}", which is not one of ${COST_BASIS_WORDS.join(", ")} — Cost not ingested (see CONVENTIONS-techniques.md)`,
          );
        } else if (Object.keys(pendingMeta.cost).length === 0) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} has a **Cost:** line with no usable pair — Cost not ingested`,
          );
        } else {
        ...
        console.warn(
          `[extract] ${sourcePath}: technique ${currentTech.name} has a **Cost basis:** line but no **Cost:** line — ignored`,
        );
      }
      // Claims ride edges, but whether the page states them rides the node:
      // "stated", "none", or absent (unknown). A line refused for its
      // grammar, or with no usable basis, leaves the technique unknown.
      let claims: Claim[] | undefined;
      let claimsBasis: ClaimsBasis | undefined;
      if (pendingMeta.claims !== undefined) {
        const basis = pendingMeta.claimsBasis;
        if (basis === undefined) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} has a **Claims:** line but no **Claims basis:** line — Claims not ingested, so its claims read as unknown (see CONVENTIONS-techniques.md)`,
          );
        } else if (!(CLAIMS_BASIS_WORDS as readonly string[]).includes(basis)) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} has claims basis "${basis}", which is not one of ${CLAIMS_BASIS_WORDS.join(", ")} — Claims not ingested (see CONVENTIONS-techniques.md)`,
          );
        } else {
          claims = pendingMeta.claims;
          claimsBasis = basis as ClaimsBasis;
        }
      } else if (pendingMeta.claimsBasis !== undefined && !pendingMeta.claimsRefused) {
        console.warn(
          `[extract] ${sourcePath}: technique ${currentTech.name} has a **Claims basis:** line but no **Claims:** line — ignored`,
        );
      }
      entities.push({
        type: "technique",
        ...
        ...(claims !== undefined && claimsBasis !== undefined
          ? {
              claims_stated: claims.length > 0 ? ("stated" as const) : ("none" as const),
              claims_basis: claimsBasis,
            }
          : {}),
      });
      ...
      for (const d of pendingMeta.demands ?? []) {
        if (!(d in DEMAND_VOCABULARY)) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} demands unknown resource "${d}" — not ingested (see CONVENTIONS-techniques.md)`,
          );
          continue;
        }
        entities.push({
          type: "technique_demands",
          technique: currentTech.name,
          resource: d,
          description: DEMAND_VOCABULARY[d],
        });
      }
      ...
      for (const r of pendingMeta.requires ?? []) {
        if (!TECHNIQUE_NAME.test(r)) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} requires "${r}", which is not a snake_case technique name — not ingested (see CONVENTIONS-techniques.md)`,
          );
          continue;
        }
        if (r === currentTech.name) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} lists itself under **Requires:** — not ingested`,
          );
          continue;
        }
      ...
      if (pendingMeta.region && pendingMeta.region !== "both") {
        entities.push({
          type: "technique_requires_region",
          technique: currentTech.name,
          region: pendingMeta.region,
        });
      }
    ...
    for (const line of lines) {
      const h2 = TECHNIQUE_H2.exec(line);
      if (h2) {
        flush();
        currentTech = {
          name: h2[1],
      ...
      const c = COMPLEXITY_LINE.exec(line);
      if (c) {
        currentTech.complexity = c[1];
        continue;
      }
      const r = REGION_LINE.exec(line);
      if (r) {
      ...
      const ur = USES_REGISTERS.exec(line);
      if (ur) {
      ...
      const uk = USES_KERNAL.exec(line);
      if (uk) {
      ...
      const dm = DEMANDS_LINE.exec(line);
      if (dm) {
      ...
      const rq = REQUIRES_LINE.exec(line);
      if (rq) {
      ...
      const rb = RASTER_BAND_LINE.exec(line);
      if (rb) {
        const band = parseRasterBand(rb[1]);
        if ("error" in band) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} has **Raster band:** ${JSON.stringify(rb[1].trim())}: ${band.error} — band not ingested, so the technique conflicts as if it had none (see CONVENTIONS-techniques.md)`,
          );
        } else {
      ...
      const clb = CLAIMS_BASIS_LINE.exec(line);
      if (clb) {
        pendingMeta.claimsBasis = clb[1].trim().replace(/`/g, "");
        continue;
      }
      const cl = CLAIMS_LINE.exec(line);
      if (cl) {
        const parsed = parseClaims(cl[1]);
        if ("error" in parsed) {
          console.warn(
            `[extract] ${sourcePath}: technique ${currentTech.name} has **Claims:** ${JSON.stringify(cl[1].trim())}: ${parsed.error} — Claims not ingested, so its claims read as unknown (see CONVENTIONS-techniques.md)`,
          );
          pendingMeta.claimsRefused = true;
        } else {
          pendingMeta.claims = parsed;
        }
        continue;
      }
      const cb = COST_BASIS_LINE.exec(line);
      if (cb) {
        pendingMeta.costBasis = cb[1].trim().replace(/`/g, "");
        continue;
      }
      const co = COST_LINE.exec(line);
      if (co) {
        ...
        for (const pair of pairs) {
          const m = COST_PAIR.exec(pair);
          const eq = pair.indexOf("=");
          const key = (eq >= 0 ? pair.slice(0, eq) : pair).trim();
          const val = eq >= 0 ? pair.slice(eq + 1).trim() : "";
          if (!(key in COST_VOCABULARY)) {
            console.warn(
              `[extract] ${sourcePath}: technique ${techName} has cost key "${key}", which is not in the cost vocabulary — pair skipped (see CONVENTIONS-techniques.md)`,
            );
            continue;
          }
          if (!m || !INTEGER.test(val) || Number(val) < 0) {
            console.warn(
              `[extract] ${sourcePath}: technique ${techName} has cost ${key}=${JSON.stringify(val)}, which is not a non-negative integer — pair skipped (see CONVENTIONS-techniques.md)`,
            );
            continue;
          }
          ...
          if (ceiling !== undefined && Number(val) > ceiling) {
            console.warn(
              `[extract] ${sourcePath}: technique ${techName} has cost ${key}=${val}, above the hardware maximum of ${ceiling} — pair skipped (see CONVENTIONS-techniques.md)`,
            );
            continue;
          }
          cost[key] = Number(val);
  ...
  if (content.includes(PITFALL_MARKER)) {
    ...
    for (const section of splitH2Sections(rest)) {
      const headerMatch = PITFALL_H2.exec(section.heading);
      ...
      if (mitigators) {
        const seen = new Set<string>();
        for (const t of mitigators
          .split(",")
          .map((s) => s.trim().replace(/`/g, ""))
          .filter(Boolean)) {
          if (!TECHNIQUE_NAME.test(t)) {
            console.warn(
              `[extract] ${sourcePath}: pitfall ${name} is mitigated by "${t}", which is not a snake_case technique name — not ingested (see CONVENTIONS-pitfalls.md)`,
            );
            continue;
          }
  ...
  if (content.includes(ARCHETYPE_MARKER)) {
    ...
    if (!ARCHETYPE_KINDS.has(kind)) {
      console.warn(
        `[extract] ${sourcePath}: archetype doc kind "${kind}" is not game or demo — no archetypes ingested from this file (see CONVENTIONS-archetypes.md)`,
      );
      return entities;
    }
    ...
    const nameList = (line: string | undefined, archetype: string, label: string): string[] => {
      ...
      for (const raw of line.split(",")) {
        ...
        if (!TECHNIQUE_NAME.test(n)) {
          console.warn(
            `[extract] ${sourcePath}: archetype ${archetype} lists "${n}" under ${label}, which is not a snake_case name — not ingested (see CONVENTIONS-archetypes.md)`,
          );
          continue;
        }
    ...
    for (const section of splitH2Sections(rest)) {
      ...
      if (!nameM) {
        ...
        if (ARCHETYPE_FINGERPRINT.test(section.body) || ARCHETYPE_PITFALLS.test(section.body)) {
          console.warn(
            `[extract] ${sourcePath}: H2 "${title}" has a fingerprint or pitfall line but no **Archetype:** line — not ingested (see CONVENTIONS-archetypes.md)`,
          );
        }
        continue;
      }
      ...
      if (seenNames.has(name)) {
        console.warn(
          `[extract] ${sourcePath}: archetype name "${name}" appears under two H2s — second one not ingested`,
        );
        continue;
      }
      seenNames.add(name);
