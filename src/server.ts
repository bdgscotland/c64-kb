...
Unit claims (**Claims:** lines, CLAIMS edges to HardwareUnit nodes): unit_contention (hard: both own the same unit, e.g. two raster-IRQ owners, since there is one raster compare, or two owners of sprite_0..7); zero_page_overlap (hard, soft when either side is relocatable; 'shared' lists the bytes); unit_shared (soft: one owns and the other shares, e.g. SFX on voice 2 beside a music player); unit_read_while_driven (soft: one reads a port the other drives); init_order (info: one uses the unit once at start-up; info does not change the verdict). unit_contention and zero_page_overlap do not run between a technique and its own prerequisite (it holds the unit through it); the soft and info rules still do. data_coverage carries claims: stated | none | unknown per technique, and the text says for how many inputs claims are stated; an unknown claim set is never treated as "claims nothing", so a unit conflict with it cannot be ruled out.
...
Output: {brief, proposed_techniques[], compatibility{conflicts, warnings, shared_infrastructure}, pitfalls[], toolchain_split{primary, cycle_tight_handoff[], rationale}, build_order[{step, label, recipes[]}], budget{region, frame_cycles, cycles_per_frame_sum, cycles_verdict, ram_budget_bytes, bytes_sum, bytes_verdict, contributors[{name, cycles_per_frame?, bytes?, basis}], without_cost[], weakest_basis, is_floor, assumptions[]}}. 'brief' is a one-sentence summary. 'proposed_techniques' carry register/KERNAL sets and implementing recipe names. 'toolchain_split.primary' is always "oscar64"; 'cycle_tight_handoff' names the raster/effect techniques handed to KickAssembler. 'budget' adds the plan up from each technique's **Cost:** line: cycles_per_frame summed against the region's frame (PAL 19,656; NTSC 17,095; PAL unless every region-locked technique is NTSC) and bytes_code + bytes_data summed against a stated RAM budget of 38,911 bytes ($0801-$9FFF). Verdicts are "over" or "under", or "no_data" when nothing contributed. 'without_cost' names the proposed techniques with no Cost line; when it is non-empty 'is_floor' is true and both sums are floors. 'weakest_basis' is the least trustworthy basis word among the contributors (measured-vice > derived-listing > arithmetic > estimated). With 'archetype' given, 'archetype' {name, title, kind, features[], risks[]} repeats what the graph holds, or 'archetype_not_found' {requested, known[]} lists every Archetype name the graph has (game and demo) when the name matches none; the plan is then built from the description alone and nothing is guessed.
